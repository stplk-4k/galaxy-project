<template>
  <div class="feedback">
    <div class="container">
      ...
      <div v-else>
        <p>Чтобы оставить отзыв, вам необходимо <router-link to="/enter">войти в систему</router-link>.</p>
      </div>
...
import { isLoggedIn as checkLoginStatus } from '@/store/auth.js';
...
export default {
  ...
  setup() {
    ...
    async function submitForm() {
      if (!isLoggedIn.value) {
        Swal.fire('Ошибка!', 'Вы должны быть авторизованы для добавления отзыва.', 'error');
        return;
      }
